...
function buildSlideInsertionPrompt({
  ...
  userText +=
    ...
    `- **Typography: templates own sizing.** Use the canonical slide-template classes (.h-title, .slide-body-text, .hero-stat-value, .eyebrow-tag, .mono-block) and let slide.css + pipeline-slide-contract.css pick the size. Do NOT add inline \`font-size\` to those elements unless content density or rendered overlap genuinely demands it. If a headline wraps unattractively, a hero stat clips, or two text blocks would overlap, you may reduce a specific element's \`font-size\` via inline style — reduce intelligently (go small enough to clear the overlap or fit on one line, but stay readable). There is no hard floor or ceiling; the pipeline no longer enforces 24px-minimum or per-template maximums.\n` +
