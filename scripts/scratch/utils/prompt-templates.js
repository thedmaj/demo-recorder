...
function buildSlideInsertionPrompt({
  ...
  const system =
    ...
    `For every label/value, key/stat, or field-result pair (e.g. API-result rows like "KYC check → Success", "STATUS → …"), visual SEPARATION is required — jammed label+value text is a rendering defect. The DEFAULT primitive is the field-row: <div class="sc-field-row"><span class="sc-field-key">LABEL</span><span class="sc-field-value">VALUE</span></div> (flex, justify-content:space-between + gap — label left, value right). Any equivalent flex/grid row with explicit gap or space-between is acceptable when a layout calls for something else; what is NEVER acceptable is a label span and a value span adjacent inside a plain <div> — they jam together with no space. ` +
  ...
  userText +=
    ...
    `- Inner structure: .frame > .chrome-logo + .eyebrow-tag + .h-title + template body in .slide-stack (T1 title may omit eyebrow). **NEVER add .chrome-foot** — pipeline slides have NO footer row. Footer-style partnership labels overlap body copy on the slide canvas and trigger a build-QA blocker. WRONG: \`<div class="chrome-foot"><span>Plaid × {brand}</span></div>\`. RIGHT: put "Plaid × {brand} · {section}" inside .eyebrow-tag only.\n` +
    `- Headline (default deck idiom): sentence case, ending with a period, with one <em> Bowery Street italic accent in .h-title. Deviate only deliberately (e.g. a stat-led or question headline that reads better without the accent) — never add MULTIPLE <em> accents or title-case shouting.\n` +
    ...
    `- **Mint budget (default 3 — deviate only with a reason):** keep references to \`--plaid-teal-500\` / \`#42F0CD\` to ~3 per slide (counted across class names, inline styles, and CSS), reserved for ONE primary eye-draw moment (a single stat, a single CTA accent, or a single chip) — that is what preserves visual hierarchy. You MAY exceed the budget when the design genuinely calls for it (e.g. a mint-themed comparison where the accent IS the story), but never as scattered decoration. For supporting text, use \`var(--plaid-white)\` or \`rgba(255,255,255,0.78)\` on navy; \`var(--plaid-ink-900)\` on light/cream/holo. **WRONG**: 8+ mint references sprinkled across cards. **RIGHT**: mint clustered on the focal element. QA flags budget overruns as a warning.\n` +
    ...
    `- **Vertical fit (HARD — build-QA blocker):** ALL content must fit inside the slide canvas at 1440×900 — no card, row, chart bar, or score chip may extend past the .slide-root edge (clipped content like a 4th field-score row cut off at the bottom is a scored defect). The FIT is the hard requirement; HOW you achieve it is your call — typical moves (examples, not mandates): reduce .slide-stack gap (32px → 20px works well), trim row padding, step the row value font down one notch, or drop the least-essential row. Fewer, denser rows beat clipped rows.\n` +
