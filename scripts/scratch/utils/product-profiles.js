...
const PRODUCT_FAMILIES = {
  ...
  plaid_protect: {
    ...
    accuracyRules: [
      "Plaid Protect is an UMBRELLA solution, not a single API. It packages Plaid Signal, Identity Verification (IDV), Monitor, Trust Index / Ti2, and Dashboard-configured rulesets under one contract.",
      "There is NO 'protect' product string for /link/token/create. Sending products: ['protect'] returns an error. Use the component strings instead: 'signal' for transaction scoring, 'identity_verification' for IDV (separate token flow), 'monitor' for watchlist screening. References to 'protect_linked_bank' / 'protect_transactions' appear in some AskBill answers but are NOT in the public Link token docs — treat as NDA / Sales-enablement only.",
      "Transaction scores are retrieved via POST /signal/evaluate (same endpoint as standalone Signal). Documented response fields: scores.customer_initiated_return_risk.score, scores.bank_initiated_return_risk.score, scores.cash_advance.score (when provisioned). Range 1–99 with higher = higher risk for all three.",
      "Pre-Auth (Confidence) Score, when enabled, appears as scores.pre_auth_confidence with INVERTED direction (higher = more confidence). Beta in many tenants — verify enablement before featuring on screen.",
      "Decision feedback uses POST /signal/decision/report with { client_transaction_id, initiated, days_funds_on_hold? } — the ONLY documented feedback endpoint for Protect/Signal flows. There is no Protect-specific report endpoint.",
      "ruleset.result values documented today: ACCEPT, REROUTE, REVIEW. REJECT is NOT a documented value — use REROUTE for declined narratives or render the host-app decision outside the API panel.",
      "Webhook event names (literal — do NOT paraphrase): SIGNAL_SCORE_READY, SIGNAL_RULE_TRIGGERED, and PROTECT_TX_MONITOR_RULE_TRIGGERED (when Protect Transaction Monitoring is enabled). webhook_type is 'SIGNAL' for the Signal events.",
      "Explainability is delivered through `core_attributes` (80+ key/value behavioral signals) and `ruleset.triggered_rule_details.internal_note`. There is NO documented `reason_codes[]` array on Signal/Protect responses — never fabricate one.",
      "Trust Index / Ti2 is in LIMITED AVAILABILITY (Ti2 shipped Oct 2025; LA cohort opened March 2026). Range is 1–100, higher = SAFER user (opposite direction from Signal scores). Trust Index response fields (`trust_index.*`, sub-scores, `/protect/*` endpoints) are NOT in public docs — demos must reference Trust Index by name + score only, without fabricating API field shapes. Plaid Sales enablement is required.",
      'Internal GTM references (Glean): Closed-Won at Albert, Credit Genie, Cash App (POC); active pipeline at Gemini, Cherry, Revolut, Oportun, Kalshi. Native iOS/Android Trust Index SDKs shipped May 14, 2026.',
    ],
    critiqueRules: [
      "Plaid Protect demos must anchor the score reveal to a named ruleset.result (ACCEPT / REVIEW / REROUTE — never REJECT) plus 2–3 named core_attributes or matched rule names. Avoid generic 'fraud check passed' framing.",
      "Do NOT show a fabricated `reason_codes: [...]` array in the API panel. Use core_attributes or ruleset.triggered_rule_details.internal_note for explainability.",
      "Trust Index may be named on screen for Plaid Protect demos that explicitly include it, but the API panel must NOT contain made-up /protect/* endpoints or Ti response shapes. Real Ti2 API surfaces are NDA / private docs.",
      "Use the canonical Signal scores' direction: higher = higher RISK. The Pre-Auth Confidence score (when used) is the only inverted one — flag it explicitly in narration if shown.",
      "Do NOT mix Plaid Protect framing with Cash Advance / EWA Score framing — EWA demos route to family `cash_advance_score` and `inputs/products/plaid-ewa-score.md`. Protect demos focus on the umbrella story (multiple scores + ruleset + Trust Index), not single-score repayment decisions.",
    ],
  },
  cash_advance_score: {
