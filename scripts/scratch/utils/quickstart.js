...
const KNOWN_PRODUCTS = [
  ...
  { slug: 'transactions',       label: 'Plaid Transactions',       hint: 'historical + recurring transaction sync' },
  { slug: 'liabilities',        label: 'Plaid Liabilities',      hint: 'student loans, mortgages, credit accounts' },
  { slug: 'assets',             label: 'Plaid Assets',             hint: 'balances + holdings via standard Plaid Link (not CRA)' },
  { slug: 'investments',        label: 'Plaid Investments',      hint: 'brokerage holdings + investment accounts' },
  { slug: 'investments-move',   label: 'Plaid Investments Move', hint: 'ACATS / POST /investments/auth/get transfers' },
...
function fillTemplateFromAnswers(answers, opts = {}) {
  ...
  const wizardHeader =
    ...
    `        Open \`inputs/quickstart-agent-bootstrap.txt\` or \`inputs/quickstart-research-task.md\`\n` +
    `        in Cursor / Claude Code (Agent mode) to run AskBill + Glean and refine the body.\n\n`;
  ...
  body = body.replace(
    /«e\.g\. Plaid Auth, Plaid Assets, Plaid Transactions, Plaid Liabilities, Plaid Investments, Plaid Signal»/,
    productLabels
  );
...
function buildResearchTaskMarkdown(answers, opts = {}) {
  ...
  const step6Auto = opts.buildAfter !== false;
  ...
  return (
    ...
    `---\n\n` +
    `## STEP 6 — Run the app-only pipeline (build-qa)\n\n` +
    (step6Auto
      ? `The user opted into **run build after research**. Once Step 5 passes, execute in the **integrated terminal** right away:\n\n` +
        '```bash\n' +
        demoCmd +
        '\n```\n\n' +
        `Monitor output in this terminal; use \`npm run pipe -- logs --follow\` in another shell if needed.\n`
      : `When the user is ready to build, they should run:\n\n` +
        '```bash\n' +
        demoCmd +
        '\n```\n\n' +
        `Summarize what you changed in \`inputs/prompt.txt\` and remind them to run the command above.\n`) +
    `\n---\n\n` +
