...
| **Duplicate JSON rail** | Do not embed JSON snippets inside `.slide-root` when `step.apiResponse` exists — use global `#api-response-panel` only. **Exception:** a technical slide the user prompt EXPLICITLY requests with a "Sample API response" JSON rail (`slideRole: api-field-reveal`) may carry ONE compact styled sample block (~8–12 pretty-printed mono lines mirroring that step's response fields) — it is the slide's PPT-export differentiator vs its host insight beat. Full raw payloads stay in the global panel. |
