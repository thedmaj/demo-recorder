...
- [high] {"claim":"CRA Base Report, LendScore, Network Insights, and Cash Flow Insights ride the same single Plaid Layer connection, so layering richer underwriting on cash-flow data reuses one connection — no second consent.","status":"high"}
- [high] {"claim":"Qualify enables an identity-only, no-bank prequalification (bureau_pull_type, is_eligible, early LendScore) up-funnel, then a Layer session enhances the decision with bank-verified data — one continuous waterfall.","status":"high"}
