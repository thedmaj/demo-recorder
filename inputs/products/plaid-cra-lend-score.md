...
2. **Hero host step** (e.g. `lendscore-reveal`): white/light host chrome; the **outcome status** ("Approved" / "Pre-qualified") plus at most ONE headline — the humanized score (e.g. "LendScore 82") — per the **HOST = STATUS-ONLY rule (2026-07-10 directive, whole CRA family: `plaid-cra-base-report.md` § Demo UI Guidance)**. Reason-code chips and any field-level detail move to the Plaid slide (and the JSON panel) — do NOT render them as host-card rows. (Supersedes the earlier Zip-era contract that put 2–3 reason-code chips on the host hero.)
